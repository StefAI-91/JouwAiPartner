...
export const feedbackProgressTemplate: FeedbackTemplate = ({ issue, portalUrl }) => {
  const title = pickIssueTitle(issue);
  const url = feedbackDeeplink(portalUrl, issue);
  const subject = "We zijn ermee aan de slag";
  const body = `Je verzoek is opgepakt. Je kunt de voortgang volgen in het portaal.`;
