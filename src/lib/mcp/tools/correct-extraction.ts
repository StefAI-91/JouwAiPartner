...
export function registerCorrectExtractionTools(server: McpServer) {
  server.tool(
    ...
    async ({ extraction_id, content, metadata, corrected_by_name }) => {
      ...
      // 1. Fetch the current extraction
      ...
      // 2. Look up the corrector's profile by name
      ...
      if (metadata) {
        // Merge new metadata with existing, only overwriting provided fields
        const existingMeta =
