...
| Platform actions                          | `tasks`, `management-insights`, `summaries`, `segments`, `scan-needs`, `weekly-summary`, `team`, `dev-detector`, `_utils` | `import`, `slack-settings`, `attachments`, `bulk-cluster-cleanup`, `widget-domains`, `widget` (route-only) | `auth`, `feedback`, `inbox`, `preferences`                         |
