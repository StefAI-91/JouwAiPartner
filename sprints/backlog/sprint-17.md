...
const CURATOR_TOOLS: Anthropic.Tool[] = [
  {
    ...
    description:
      "Find content entries with embedding similarity above a threshold. Used for duplicate detection.",
    input_schema: {
      ...
      properties: {
        ...
        threshold: {
          type: "number",
          description: "Similarity threshold (0.0-1.0). Use 0.90 for duplicates.",
        },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        days_threshold: {
          type: "number",
          description: "Content older than this many days without review",
        },
      },
  ...
  switch (name) {
          ...
          if (similarity > input.threshold) {
            pairs.push({
              entry_a: {
                id: data[i].id,
                title: data[i].title,
                content: data[i].content?.slice(0, 200),
              },
              entry_b: {
                id: data[j].id,
                title: data[j].title,
                content: data[j].content?.slice(0, 200),
              },
    ...
    case "find_stale_content": {
      const cutoff = new Date(
        Date.now() - input.days_threshold * 24 * 60 * 60 * 1000,
      ).toISOString();
      const { data } = await supabase
        .from(input.table)
  ...
  const messages: Anthropic.MessageParam[] = [
    {
      role: "user",
      content:
        "Run the nightly database hygiene checks. Process all content tables: documents, meetings, slack_messages, emails.",
    },
  ];
...
Deno.serve(async (req) => {
  ...
  const response = await fetch(`${Deno.env.get("APP_URL")}/api/agents/curator`, {
    method: "POST",
