# Sprint 19: MCP Server — Expanded Tools
  ...
  server.tool(
    ...
    "Find people by skill, project involvement, or general semantic search. Use when someone asks 'who knows X?' or 'who worked on Y?'",
    {
      query: z
        .string()
        .optional()
        .describe("Semantic search query (e.g., 'React Native experience')"),
  ...
  server.tool(
    ...
    async ({ person_id, name }) => {
      ...
      const { data: skills } = await supabase
        .from("people_skills")
        .select("skill, evidence_count, last_seen")
        .eq("person_id", person.id)
        .order("evidence_count", { ascending: false });

      const { data: projects } = await supabase
        .from("people_projects")
        .select("project, role_in_project, last_mentioned")
        .eq("person_id", person.id)
      ...
      const profile = [
        ...
        skills
          ?.map(
            (s) =>
              `- ${s.skill} (${s.evidence_count} mentions, last: ${s.last_seen ? new Date(s.last_seen).toLocaleDateString() : "unknown"})`,
          )
          .join("\n") || "No skills recorded.",
        "",
        ...
        projects
          ?.map(
            (p) =>
              `- ${p.project} (${p.role_in_project || "contributor"}, last: ${p.last_mentioned ? new Date(p.last_mentioned).toLocaleDateString() : "unknown"})`,
          )
  ...
  server.tool(
    ...
    async ({ project_name }) => {
      ...
      const sections = [
        ...
        `### Team (${team?.length || 0})`,
        team
          ?.map((t: any) => `- ${t.people?.name} (${t.role_in_project || "contributor"})`)
        ...
        decisions
          ?.map(
            (d) =>
              `- ${d.decision} (${d.made_by || "Unknown"}, ${d.date ? new Date(d.date).toLocaleDateString() : "Unknown"})`,
          )
        ...
        actions
          ?.map(
            (a) =>
              `- [${a.status}] ${a.description} → ${a.assignee || "Unassigned"}${a.due_date ? ` (due: ${a.due_date})` : ""}`,
          )
          .join("\n") || "No action items.",
      ];
