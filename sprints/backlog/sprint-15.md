...
async function watchGmail(accessToken: string, userEmail: string): Promise<void> {
  const response = await fetch(`https://gmail.googleapis.com/gmail/v1/users/${userEmail}/watch`, {
    method: "POST",
    headers: {
...
Deno.serve(async (req) => {
  ...
  const accessToken = await getAccessToken(Deno.env.get("GOOGLE_SERVICE_ACCOUNT_KEY")!, userEmail);
