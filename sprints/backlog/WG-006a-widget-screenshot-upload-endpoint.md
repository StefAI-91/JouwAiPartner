...
| WG-REQ-113 | Acceptance-tests (vitest in `apps/devhub/__tests__/api/ingest-widget-screenshot.test.ts`, dezelfde stijl als de bestaande userback-route-tests): valid PNG → 200 + token; te grote file → 413; verkeerde mime → 415; ongeautoriseerde Origin → 403; 11e upload binnen uur → 429; CORS-preflight → 204. End-to-end via Playwright komt in WG-006b zodra de widget echt screenshots stuurt. |
