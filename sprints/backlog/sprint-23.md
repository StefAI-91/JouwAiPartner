# Sprint 23: Analyst Agent

**Phase:** V3 — Intelligentie & Frontend
**Requirements:** REQ-800–REQ-806
**Depends on:** Sprint 17 (Curator pattern established), Sprint 02 (insights table)
...
const ANALYST_TOOLS: Anthropic.Tool[] = [
  {
    ...
    description:
      "Search content ingested in the last N days. Returns semantically relevant results.",
    input_schema: {
...
export async function runAnalyst(): Promise<string> {
  const messages: Anthropic.MessageParam[] = [{ role: "user", content: ANALYST_PROMPT }];
