...
1. Bestaat tabel `communications` met alle gedeelde kolommen (party_type, organization_id, unmatched_organization_name, scope, occurred_at, relevance_score, verification_status, verified_by, verified_at, embedding, embedding_stale, search_vector, created_at, updated_at).
...
5. Is er één `PARTY_TYPES` enum in `packages/ai/src/validations/communication.ts` die de unie van beide huidige enums bevat (`internal, client, partner, accountant, tax_advisor, lawyer, advisor, supplier, other`).
...
| DATA-073 | CHECK constraint op `communications.party_type` accepteert alleen waardes uit de gedeelde enum (`internal, client, partner, accountant, tax_advisor, lawyer, advisor, supplier, other`). CHECK op `meetings.party_type` en `emails.party_type` worden in déze sprint verbreed naar dezelfde enum zodat dual-write niet faalt.                                                                                                                                                                                                                                                                                                       |
...
- [ ] Nieuw: `packages/ai/src/validations/communication.ts`
  - `export const PARTY_TYPES = ['internal', 'client', 'partner', 'accountant', 'tax_advisor', 'lawyer', 'advisor', 'supplier', 'other'] as const`
...
- [ ] `supabase/migrations/YYYYMMDD_comm001_backfill_communications.sql`
  ...
  - scope: default `'project'` als er een project-koppeling bestaat via `meeting_projects`/`email_projects`, anders `NULL` (gatekeeper vult 'm in COMM-002)
