...
- Update `CLOSED_STATUSES` (regel 39): voeg `'declined'` en `'converted_to_qa'` toe; `'deferred'` blijft buiten.
...
- **`components/inbox-list.tsx`** — gegroepeerde rendering:
...
- `apps/cockpit/src/lib/constants/navigation.ts`:
  ...
  - **Regel 22-29:** voeg toe in `primaryNavItems`: `{ href: "/inbox", label: "Inbox", icon: Inbox, badgeKey: "inboxCount" }` (`Inbox` icon importeren uit `lucide-react`).
...
Drie wijzigingen, allemaal één regel:

- `apps/portal/src/actions/feedback.ts:58` — `status: "triage"` → `status: "needs_pm_review"`.
- `packages/database/src/mutations/widget/feedback.ts:39` — idem.
- Userback-import-pad: bevestig caller-keten met `grep -rn "insertIssue\|upsertUserbackIssues" packages/database/src/`. Voor nieuwe inserts vanuit Userback óók flippen naar `needs_pm_review`. Bestaande imports blijven onveranderd (geen backfill).
...
- **Constants-test** (`packages/database/src/constants/__tests__/issues.test.ts`): alle nieuwe statussen hebben een label én zitten in een portal-status-groep (geen wees-status).
