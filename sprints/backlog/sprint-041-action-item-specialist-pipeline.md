...
1. Draait de Action Item Specialist automatisch in de gatekeeper-pipeline op iedere nieuwe meeting, parallel aan de RiskSpecialist, op de **Fireflies-transcript** (`input.transcript`) — niet de named/elevenlabs versie. Reden: letterlijke `source_quote`-validatie heeft voorrang boven transcriptie-kwaliteit zolang speaker-mapping nog drift introduceert (zie `docs/stand-van-zaken.md` regels 95, 159, 165).
...
| AI-051b  | Step gebruikt **Fireflies-transcript** (`input.transcript`) als input — niet `namedTranscript` of `transcribeResult.transcript`. RiskSpecialist en Summarizer blijven `bestTranscript` gebruiken; specialist-keuze is bewust afwijkend. Heroverwegen wanneer speaker-mapping consistent ≥95% naam-attributie haalt (open punt in stand-van-zaken regel 173) |
...
| Transcript        | **Fireflies (`input.transcript`)** — bewust afwijkend van RiskSpecialist + Summarizer (die gebruiken `bestTranscript`). Reden: letterlijke `source_quote`-matching tegen golden was te onbetrouwbaar op named-transcript; speaker-mapping geeft drift. Zie `docs/stand-van-zaken.md` regels 95 + 159 + 165. Heroverwegen wanneer speaker-mapping ≥95% naam-attributie haalt |
...
4. **Transcript-keuze divergeert van andere specialists** ⚠️ — RiskSpecialist + Summarizer draaien op `bestTranscript` (named first), action-item-specialist op `input.transcript` (Fireflies). Bewust per `docs/stand-van-zaken.md` regel 159: letterlijkheid > kwaliteit voor source_quote-validatie. Trigger om te heroverwegen: speaker-mapping consistent ≥95% naam-attributie + voldoende productie-meetings waar Fireflies-attributie items mist die ElevenLabs wél vangt. Eén regel flippen in de step is dan genoeg.
...
const actionItemPromise = runActionItemSpecialistStep(     // NIEUW
  meetingId,
  input.transcript,                                         // Fireflies, NIET bestTranscript
  {
    title: input.title,
