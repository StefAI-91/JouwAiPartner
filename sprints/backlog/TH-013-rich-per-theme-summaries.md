...
- **Extraction-centric theme detail page.** Overwogen in de research-thread, bewust niet gekozen. Reden: de bestaande `meeting_themes.summary` + de Summarizer-upgrade lossen de gesignaleerde bleed-pijn op zonder datamodel-shift. Extraction-centric blijft een optie voor later als we discrete facts cross-meeting willen synthetiseren — dan is de MeetingStructurer-sprint de voorwaarde.
- **MeetingStructurer reanimeren.** De PW-02 commits introduceerden een agent die transcript → typed extraction rows (decision/action_item/insight/etc.) produceerde; die agent is tussentijds weer weggehaald. Of hij terugkomt is een aparte strategische sprint — TH-013 werkt met de huidige pipeline die alleen `need` + `risk` als AI-extracted rows in de tabel zet.
- **NeedsScanner van summary naar transcript.** De user wil "transcript voor extractions, niet summary" — voor needs is dat nog niet geregeld (scanner gebruikt `meeting.summary`). Aparte sprint, onafhankelijk van deze. Niet bundelen om scope-creep te voorkomen.
- **Theme-Detector pipeline-volgorde verschuiven.** De Detector draait nu vóór de Summarizer en gebruikt de Fireflies-ruwe summary als input. Dat blijft zo; TH-013 lost de UI-pijn op via het per-thema output van de Summarizer, niet via betere Detector-input. Als de Detector-kwaliteit zelf onvoldoende blijkt (meetings die ten onrechte wel/niet matchen), is dat een eigen sprint.
- **Gestructureerde kolommen voor theme-content (optie B).** `meeting_themes.summary` blijft één markdown-string. Splitsing in `briefing text + kernpunten jsonb + vervolgstappen jsonb` is pas zinvol als een AI-consumer per-veld wil queryen. Geen hypothetische voorbereiding.
- **Fallback-indicator (badge/icon) op theme-kaart.** UI-341 houdt fallback-rijen visueel identiek aan primaire rijen. Als gebruikers later expliciet willen weten welk model schreef, is dat een aparte UX-sprint — niet in V1 om de kaart niet te vervuilen met meta-info.
- **Dedicated per-theme summarizer agent.** De user heeft bewust gekozen voor één Summarizer-call met extra output-veld i.p.v. N aparte calls. Mocht de kwaliteit tegenvallen (multi-output-degradatie meetbaar onder baseline) dan is de dedicated-route een latere upgrade — zonder weg-terug-verlies want het schema staat al los van de uitvoeringsstrategie.
- **Theme-summaries voor emails.** `email_extractions` + het bestaande email-pipeline hebben hun eigen summarizer-pad zonder theme-scoping. Per-thema emails is een apart ontwerp (polymorphic theme-junction of email-specifiek veld). Niet in TH-013.
- **Partial-backfill filter op meest-bekeken themes.** Zou kleinere batches toelaten voor de 5 "heetste" thema's eerst, vereist een theme-filter in `scripts/batch-detect-themes.ts`. Uit scope — keuze is pad 1 (volledig) of pad 2 (rolling), geen tussenweg.
- **Annotation-only regenerate.** Een lichte `RegenerateMenu`-variant die alleen link-themes herdraait op bestaande Summarizer-output (zonder nieuwe AI-call). Kan als optimalisatie later, maar vereist dat `meetings.summary` al TH-013-contract is — geldt pas voor post-deploy meetings. Nu wint de volle keten voor consistency.
- **Theme-Narrator — cross-meeting synthese.** Per-thema synthese over meerdere meetings (analoog aan project-summarizer) is een eigen sprint. TH-013 levert de per-meeting bouwsteen waar een toekomstige Narrator op kan staan; Narrator zelf staat er los van.
- **Telemetry-kolom voor bron-tracking op `meeting_themes`.** Zou een `summary_source: 'summarizer' | 'detector' | null` kolom betekenen. Niet toegevoegd — event-based tracking via `agent_runs.metadata.theme_summaries_missing` volstaat voor monitoring.
