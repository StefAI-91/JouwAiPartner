...
**Opnieuw aangemaakt (nieuw schema, 1024-dim):** 3. `people` (email UNIQUE, embedding VECTOR(1024), embedding_stale DEFAULT TRUE) 4. `projects` (name UNIQUE, aliases, organization_id FK, embedding VECTOR(1024)) 5. `meetings` (fireflies_id UNIQUE, meeting_type, party_type, organization_id FK, search_vector TSVECTOR + trigger)

**Nieuwe koppeltabellen:** 6. `meeting_projects` (composite PK, CASCADE deletes) 7. `meeting_participants` (composite PK, CASCADE deletes)

**Nieuwe unified extractie-tabel:** 8. `extractions` (meeting_id FK CASCADE, type, confidence, transcript_ref, metadata JSONB, corrected_by/corrected_at, search_vector TSVECTOR + trigger)

**Types:** 9. Supabase TypeScript types regenereren
