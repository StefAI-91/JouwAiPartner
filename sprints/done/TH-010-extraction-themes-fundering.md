...
**Plus één dev-tool om prompt-tuning te versnellen:** `/dev/tagger` — admin-only harness die op een geselecteerde meeting de ThemeTagger in dry-run mode draait en de output zij-aan-zij toont met de huidige DB-state. Geen prompt-editor in de UI; prompt-wijzigingen blijven via code+redeploy. Deze harness is TH-010-local; zodra de bredere `/dev/extractor` (uit branch `claude/fix-risk-analyzer-confidence-0kKhj` / PW-02) naar main mergt, verhuist `/dev/tagger` als tweede tab/pane in die route.
...
| AI-226   | Starter-set constante `TAGGER_EXTRACTION_TYPES = ["decision", "action_item", "need", "insight"]` in een gedeelde constants-file (bv. `packages/ai/src/agents/theme-tagger.ts`). Pipeline-step filtert `getMeetingExtractions()` op deze types vóór Tagger-input. Progressive toevoegen = één-regel-wijziging. Prompt-regel in AI-221 meldt dat andere types al uitgefilterd zijn. |
...
| FUNC-260 | Server Action `runDevTaggerAction({ meetingId })` in `apps/cockpit/src/actions/dev-tagger.ts` — dry-run: haalt extractions + verified themes + negative_examples op, roept `tagMeetingThemes()` aan, retourneert `{ taggerOutput, currentMeetingThemes, currentExtractionThemes, systemPrompt }`. Schrijft NIKS naar DB                                                           |
| SEC-220  | `/dev/tagger` route + `runDevTaggerAction` checken admin-whitelist (Stef/Wouter emails, zelfde pattern als `approveThemeAction` uit TH-006). Non-admins krijgen 403 op de route en `{error:'forbidden'}` op de action                                                                                                                                                             |
| UI-320   | `apps/cockpit/src/app/(dashboard)/dev/tagger/page.tsx` (server component: admin-check, metadata, renders client) + `client.tsx` (client component: meeting-picker, Run-knop, input-panel met summary+extractions+themes counts, output-panel met matches/proposals/meta, collapsible system-prompt viewer)                                                                        |
| UI-321   | Client-side diff-berekening: huidige `meeting_themes` + `extraction_themes` rijen vs fresh Tagger-output. Toon per match: `+` nieuw, `−` verdwenen, `=` ongewijzigd. Geen persistente opslag — refresh reset de preview                                                                                                                                                           |
| EDGE-222 | Harness-run op meeting zonder extractions of zonder verified themes: toont duidelijke UI-melding ("geen extractions om te taggen" / "geen themes beschikbaar"), geen crash. Consistent met bestaande skip-paths in `runTagThemesStep` (EDGE-200, EDGE-201)                                                                                                                        |
...
- [ ] `apps/cockpit/src/app/(dashboard)/dev/tagger/page.tsx` — admin-guarded server wrapper
- [ ] `apps/cockpit/src/app/(dashboard)/dev/tagger/client.tsx` — harness UI (picker, Run, input/output/diff panels, system-prompt viewer)
- [ ] `apps/cockpit/src/actions/dev-tagger.ts` — `runDevTaggerAction` Server Action met whitelist-check
- [ ] Tests (nieuwe + updates):
  ...
  - [ ] Nieuw: `apps/cockpit/__tests__/actions/dev-tagger.test.ts` — whitelist-check (admin toegelaten, non-admin forbidden), dry-run schrijft niets naar DB, retourneert Tagger-output + current-state shape
...
9. Open `/dev/tagger` als admin (Stef/Wouter): meeting-picker werkt, Run-knop schiet Tagger af binnen enkele seconden, output-panel toont matches + extractionIds, diff-panel toont veranderingen tov DB. Geen nieuwe rijen in `meeting_themes` of `extraction_themes` na een harness-run (dry-run verificatie).
10. Open `/dev/tagger` als non-admin user → 403 / redirect naar dashboard.
