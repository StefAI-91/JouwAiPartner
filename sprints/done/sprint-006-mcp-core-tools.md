...
1. `search_knowledge` uitbreiden: bronvermelding (meeting titel, datum, transcript*ref) + confidence + verificatie-status
   1b. Sufficiency check in MCP system prompt: *"Als je geen relevante bronnen vindt, zeg dat expliciet. Geef nooit een antwoord zonder bron."\_ Geen code nodig — Claude respecteert dit via de system prompt.
...
- [x] search_knowledge uitbreiden met bronvermelding + confidence
- [x] Sufficiency check toevoegen aan MCP system prompt ("geen bron = zeg dat eerlijk")
- [x] get_decisions tool bouwen
- [x] get_action_items tool bouwen
- [x] get_meeting_summary tool bouwen
- [x] Verificatie-status logica (AI confidence vs corrected_by)
