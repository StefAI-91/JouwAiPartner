...
async function matchProjectEntity(name: string): Promise<{ id: string; name: string } | null> {
  ...
  if (semantic && semantic.length > 0) {
    ...
    const { data: current } = await supabase
      .from("projects").select("aliases").eq("id", project.id).single();
...
**What:** Add project entity deduplication to the Curator's nightly sweep (already partially done in Sprint 15/16). Ensure the Curator checks for project aliases that should be merged.
