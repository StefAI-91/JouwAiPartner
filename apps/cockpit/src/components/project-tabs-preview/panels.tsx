...
/**
 * Gedeelde mock-content voor beide project-detail previews
 * (top-tabs en linker-sidebar). Pure UI met hardcoded data —
 * geen DB, geen mutations.
 */

export type PanelKey = "overview" | "activity" | "insights" | "inbox";

export const PANEL_META: { key: PanelKey; label: string; badge?: number }[] = [
...
export function PanelContent({ panel }: { panel: PanelKey }) {
  ...
  if (panel === "activity") return <ActivityPanel />;
  if (panel === "insights") return <InsightsPanel />;
  return <InboxPanel />;
}
