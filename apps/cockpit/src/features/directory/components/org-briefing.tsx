...
interface OrgBriefingProps {
  content: string | null;
  createdAt?: string;
}

export function OrgBriefing({ content, createdAt }: OrgBriefingProps) {
  return (
    <section className="mb-6 rounded-lg bg-[#006B3F]/[0.03] px-5 py-4">
      ...
      {content ? (
        ...
        <p className="text-sm italic text-muted-foreground/60">Nog geen briefing beschikbaar</p>
      )}
    </section>
  );
}
