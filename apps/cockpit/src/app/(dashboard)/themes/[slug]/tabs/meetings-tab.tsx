...
/**
 * UI-272: Meetings-tab. Lijst gesorteerd op datum desc, per rij titel +
 * datum + participants + confidence-badge, en een `<details>` met de
 * letterlijke evidence-quote die de ThemeTagger als bewijs gaf.
 */
export function MeetingsTab({ meetings }: MeetingsTabProps) {
  ...
  return (
    <ul className="space-y-2">
      {meetings.map((m) => (
        <li key={m.meeting_id} className="rounded-xl border border-border/60 bg-card p-4">
