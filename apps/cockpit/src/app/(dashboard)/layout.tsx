...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [reviewCount, focusProjects] = await Promise.all([
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <DesktopSidebar reviewCount={reviewCount} focusProjects={focusProjects} />
      ...
      <div className="flex min-h-screen min-w-0 flex-1 flex-col">
        ...
        <header className="flex h-14 items-center gap-3 border-b border-border/50 bg-white/60 px-4 backdrop-blur-sm lg:hidden">
          <SideMenu reviewCount={reviewCount} focusProjects={focusProjects} />
          <img
