...
/**
 * Discriminated union voor alle dev-extractor Server Actions. Past bij de
 * conventie elders in cockpit: `{ success: true, data } | { success: false, error }`.
 * `success` als discriminator zodat TypeScript narrowing schoon werkt.
 */
...
export async function runDevExtractorAction(
...
): Promise<ActionResult<DevExtractorResult>> {
  ...
  try {
    ...
    return { success: false, error: `MeetingStructurer faalde: ${msg}` };
  }
}
...
> {
  const guard = await requireAdminInAction();
  if ("error" in guard) return { success: false, error: guard.error };
  return { success: true, data: { prompt: MEETING_STRUCTURER_SYSTEM_PROMPT } };
}
...
export async function runDevRiskSpecialistAction(
...
): Promise<ActionResult<DevRiskSpecialistResult>> {
  const guard = await requireAdminInAction();
  if ("error" in guard) return { success: false, error: guard.error };
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    return { success: false, error: `RiskSpecialist faalde: ${msg}` };
  }
}
 ...
 */
export async function getRiskSpecialistPromptAction(): Promise<ActionResult<{ prompt: string }>> {
  const guard = await requireAdminInAction();
  if ("error" in guard) return { success: false, error: guard.error };
  return { success: true, data: { prompt: RISK_SPECIALIST_SYSTEM_PROMPT } };
}
