...
Deze v1 ("Stefan First") richt zich op de directe pijn van Stefan Roevros (CAI): een overzichtelijk vier-bucket overzicht van issues zonder interne ruis. De vier buckets zijn `Ontvangen`, `Ingepland`, `In behandeling`, `Afgerond` (zoals bestaand in `PORTAL_STATUS_GROUPS`). Voting, sign-off en comments worden in v2 toegevoegd zonder breaking changes.

> Deze PRD vervangt `docs/specs/portal-mvp.md` (gearchiveerd). Wat in `portal-mvp.md` al opgeleverd was (auth, RLS, project-overzicht, app-scaffolding) is hier alleen ter context opgenomen — de inhoudelijke nieuwigheid zit in de `client_visible`-laag en het vier-bucket dashboard.
